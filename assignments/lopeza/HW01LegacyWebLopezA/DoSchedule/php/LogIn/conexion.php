...
session_start();
  ...
  $query = "SELECT * FROM registro_docentes WHERE do_username='$user' AND do_tipo='Estudiante'";
  ...
  if ($resultado) {
      ...
      if ($row>0) {
          //error_log($row[0], 0);
          if(password_verify($clave, $row[27])==1){
              // Guardo en la sesión el user del usuario.
            $_SESSION['user'] = $user;
            // Redirecciono al usuario a la página principal del sitio.
            registerAudit($user);
            header("Location: ../../html/LogIn/estudiante.php");
          }else{
              header("Location: ../../html/error1.html");
          }
      } else {
          
        $query2 = "SELECT * FROM registro_docentes WHERE do_username='$user' AND do_tipo='Docente'";
        $resultado2 = mysqli_query($conn,$query2);

        $row2 = mysqli_fetch_array($resultado2);
    if($row2>0){
          if(password_verify($clave, $row2[27])==1){
            // Guardo en la sesión el user del usuario.
        $_SESSION['user'] = $user;
        // Redirecciono al usuario a la página principal del sitio.
        registerAudit($user);
        $userId = $row2[0];
          header("Location: ../../html/LogIn/docente.php?profesorId=$userId");
          }else{
